...
function renderEntryCard(entry, origin) {
  const image = entry.coverImageUrl
    ? `<div class="entry-card__media"><img src="${escapeAttribute(absoluteUrl(entry.coverImageUrl, origin))}" alt="${escapeAttribute(
        entry.coverAlt || entry.title
      )}"></div>`
...
.entry-page{width:min(1120px,100%);margin:0 auto;padding:clamp(18px,4vw,52px)}.list-page{width:min(1180px,100%);margin:0 auto;padding:clamp(12px,2.4vw,28px) clamp(16px,4vw,52px) clamp(26px,4vw,52px)}.entry-article{background:var(--paper);border:1px solid var(--line);border-radius:18px;overflow:hidden;box-shadow:0 24px 70px -52px rgba(58,46,38,.6)}.entry-hero-grid{display:grid;grid-template-columns:minmax(0,1fr) minmax(340px,520px);gap:34px;align-items:start}.entry-heading{min-width:0;align-self:start}.entry-content{margin-top:26px}.entry-poster-preview{width:100%;max-width:520px;margin:0;justify-self:end;background:#fffaf1;border:1px solid var(--line);border-radius:16px;padding:10px;box-shadow:0 20px 60px -46px rgba(58,46,38,.7)}.entry-poster-preview img{width:100%;max-height:720px;object-fit:contain;display:block;background:#fffaf1}.entry-poster-preview figcaption{margin-top:8px;text-align:center;font-size:13px;font-weight:800;color:var(--muted)}.entry-poster-preview a{text-decoration:none;border-bottom:1px solid currentColor}
